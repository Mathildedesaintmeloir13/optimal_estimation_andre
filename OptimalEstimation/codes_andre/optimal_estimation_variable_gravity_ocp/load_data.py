...
def stats(subject, trial, number_shooting_points, testing_angle):
    ...
    load_name = load_path + subject + '/' + os.path.splitext(c3d_name)[0] + "_optimal_gravity_N" + str(adjusted_number_shooting_points) + "test_angle_" + str(testing_angle[1]) + '_rerun'
    ...
    root_OGE_shifts = [(((states_optimal_gravity['q'][3, :] - states['q'][3, :])/(2*np.pi)).mean().round(), 3),
                       (((states_optimal_gravity['q'][5, :] - states['q'][5, :])/(2*np.pi)).mean().round(), 5)]
    for root_OGE_shift, idx in root_OGE_shifts:
        states_optimal_gravity['q'][idx, :] -= root_OGE_shift * (2*np.pi)
    ...
    root_EKF_shifts = [(((q_kalman_biorbd[3, :] - states['q'][3, :])/(2*np.pi)).mean().round(), 3),
                       (((q_kalman_biorbd[5, :] - states['q'][5, :])/(2*np.pi)).mean().round(), 5)]
    ...
    for segment in segments.values():
        ...
        segment_marker_error_OE_all[segment['name']] = np.sqrt(np.nanmean(markers_error_OE[segment['markers_idx'], :]**2, axis=1))
        segment_marker_error_OGE_all[segment['name']] = np.sqrt(np.nanmean(markers_error_OGE[segment['markers_idx'], :]**2, axis=1))
        segment_marker_error_EKF_biorbd_all[segment['name']] = np.sqrt(np.nanmean(markers_error_EKF_biorbd[segment['markers_idx'], :]**2, axis=1))
    ...
    difference_between_Q_OGE = (states_kalman_biorbd['q'] - states_optimal_gravity['q'])
    difference_between_Q_OGE[:3, :] = difference_between_Q_OGE[:3, :] * 1000
    difference_between_Q_OGE[3:, :] = difference_between_Q_OGE[3:, :] / np.pi * 180
    difference_between_Q_OE = (states_kalman_biorbd['q'] - states['q'])
    difference_between_Q_OE[:3, :] = difference_between_Q_OE[:3, :] * 1000
    difference_between_Q_OE[3:, :] = difference_between_Q_OE[3:, :] / np.pi * 180

    RMSE_difference_between_Q_trans_OGE = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[:3, :] ** 2, axis=1)))
    RMSE_difference_between_Q_trans_OE = np.mean(np.sqrt(np.mean(difference_between_Q_OE[:3, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OGE = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[3:, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OE = np.mean(np.sqrt(np.mean(difference_between_Q_OE[3:, :] ** 2, axis=1)))

    difference_between_Qdot_OGE = (states_kalman_biorbd['q_dot'] - states_optimal_gravity['q_dot'])
    difference_between_Qdot_OGE[:3, :] = difference_between_Qdot_OGE[:3, :] * 1000
    difference_between_Qdot_OGE[3:, :] = difference_between_Qdot_OGE[3:, :] / np.pi * 180
    difference_between_Qdot_OE = (states_kalman_biorbd['q_dot'] - states['q_dot'])
    difference_between_Qdot_OE[:3, :] = difference_between_Qdot_OE[:3, :] * 1000
    difference_between_Qdot_OE[3:, :] = difference_between_Qdot_OE[3:, :] / np.pi * 180

    RMSE_difference_between_Qdot_trans_OGE = np.mean(np.sqrt(np.mean(difference_between_Qdot_OGE[:3, :] ** 2, axis=1)))
    RMSE_difference_between_Qdot_trans_OE = np.mean(np.sqrt(np.mean(difference_between_Qdot_OE[:3, :] ** 2, axis=1)))
    RMSE_difference_between_Qdot_OGE = np.mean(np.sqrt(np.mean(difference_between_Qdot_OGE[3:, :] ** 2, axis=1)))
    RMSE_difference_between_Qdot_OE = np.mean(np.sqrt(np.mean(difference_between_Qdot_OE[3:, :] ** 2, axis=1)))

    trunk_dofs = range(3, 12)
    arms_dofs = range(12, 30)
    legs_dofs = range(30, 42)

    RMSE_difference_between_Q_OGE_trunk = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[trunk_dofs, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OE_trunk = np.mean(np.sqrt(np.mean(difference_between_Q_OE[trunk_dofs, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OGE_arms = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[arms_dofs, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OE_arms = np.mean(np.sqrt(np.mean(difference_between_Q_OE[arms_dofs, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OGE_legs = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[legs_dofs, :] ** 2, axis=1)))
    RMSE_difference_between_Q_OE_legs = np.mean(np.sqrt(np.mean(difference_between_Q_OE[legs_dofs, :] ** 2, axis=1)))

    RMSE_difference_between_Q_OGE_segment = {}
    RMSE_difference_between_Q_OE_segment = {}
    for segment in segments.values():
        if segment['name'] == 'Pelvis':
            RMSE_difference_between_Q_trans_OGE_segment = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[segment['dofs'][:3]] ** 2, axis=1)))
            RMSE_difference_between_Q_trans_OE_segment = np.mean(np.sqrt(np.mean(difference_between_Q_OE[segment['dofs'][:3]] ** 2, axis=1)))
            RMSE_difference_between_Q_OGE_segment[segment['name']] = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[segment['dofs'][3:]] ** 2, axis=1)))
            RMSE_difference_between_Q_OE_segment[segment['name']] = np.mean(np.sqrt(np.mean(difference_between_Q_OE[segment['dofs'][3:]] ** 2, axis=1)))
        else:
            RMSE_difference_between_Q_OGE_segment[segment['name']] = np.mean(np.sqrt(np.mean(difference_between_Q_OGE[segment['dofs']] ** 2, axis=1)))
            RMSE_difference_between_Q_OE_segment[segment['name']] = np.mean(np.sqrt(np.mean(difference_between_Q_OE[segment['dofs']] ** 2, axis=1)))
    ...
    mean_OE = np.mean(np.abs(momentum_OE), axis=1)[:, np.newaxis]
    mean_OGE = np.mean(np.abs(momentum_OGE), axis=1)[:, np.newaxis]
    mean_EKF = np.mean(np.abs(momentum_EKF), axis=1)[:, np.newaxis]

    RMSE_momentum_OE = np.sqrt(np.mean((np.abs(momentum_OE) - mean_OE) ** 2, axis=1))
    RMSE_momentum_OGE = np.sqrt(np.mean((np.abs(momentum_OGE) - mean_OGE) ** 2, axis=1))
    RMSE_momentum_EKF = np.sqrt(np.mean((np.abs(momentum_EKF) - mean_EKF) ** 2, axis=1))
    ...
    # print('Rank: ', rank)
    print('Relative rank: ', rank.count(biorbd_model.nbQ()) / adjusted_number_shooting_points * 100)
    ...
    save_path = 'Solutions/'
    save_name = save_path + subject + '/Plots/' + os.path.splitext(c3d_name)[0] + "_momentum" + "_test_angle_" + str(testing_angle[1]) + '.png'
    ...
    save_name = save_path + subject + '/Plots/' + os.path.splitext(c3d_name)[0] + "_error" + "_test_angle_" + str(testing_angle[1]) + '.png'
    ...
    #     save_path = 'Solutions/'
    #     save_name = save_path + subject + '/Plots/' + os.path.splitext(c3d_name)[0] + '_' + dofs_name[idx_dof] + "_test_angle_" + str(testing_angle[1]) + '.png'
    ...
    save_path = 'Solutions/' + subject + '/' + os.path.splitext(c3d_name)[0] + '_stats_test_angle_' + str(testing_angle[1])
    ...
    with open(save_variables_name, 'wb') as handle:
        pickle.dump({'subject': subject, 'trial': trial, 'adjusted_number_shooting_points': adjusted_number_shooting_points,
                     ...
                     'model_labels': model_labels, 'gravity': gravity, 'gravity_angle': angle, 'testing_angle': testing_angle,
                     ...
                     'difference_between_Q_OGE': difference_between_Q_OGE,
                     'difference_between_Q_OE': difference_between_Q_OE,
                     'diff_Q_trans_EKF_OGE': RMSE_difference_between_Q_trans_OGE, 'diff_Q_trans_EKF_OE': RMSE_difference_between_Q_trans_OE,
                     'diff_Q_EKF_OGE': RMSE_difference_between_Q_OGE, 'diff_Q_EKF_OE': RMSE_difference_between_Q_OE,
                     'diff_Qdot_trans_EKF_OGE': RMSE_difference_between_Qdot_trans_OGE, 'diff_Qdot_trans_EKF_OE': RMSE_difference_between_Qdot_trans_OE,
                     'diff_Qdot_EKF_OGE': RMSE_difference_between_Qdot_OGE, 'diff_Qdot_EKF_OE': RMSE_difference_between_Qdot_OE,
                     ...
                     'RMSE_difference_between_Q_trans_OGE_segment': RMSE_difference_between_Q_trans_OGE_segment,
                     'RMSE_difference_between_Q_OGE_segment': RMSE_difference_between_Q_OGE_segment,
                     'RMSE_difference_between_Q_trans_OE_segment': RMSE_difference_between_Q_trans_OE_segment,
                     'RMSE_difference_between_Q_OE_segment': RMSE_difference_between_Q_OE_segment,
                     ...
                     'segment_marker_error_EKF_biorbd_all': segment_marker_error_EKF_biorbd_all,
                     'segment_marker_error_OGE_all': segment_marker_error_OGE_all,
                     'segment_marker_error_OE_all': segment_marker_error_OE_all,
...
if __name__ == "__main__":
    ...
    stats(subject, trial, number_shooting_points,testing_angle)
