...
if __name__ == "__main__":
    ...
    for subject_trial in subjects_trials:
        ...
        MNF_EKF_biorbd_sum.append(eng.meanfreq(signal, (frequency / 3)))

        eng.quit()

        fft_freq.append(frequency / 3 * np.arange(0, int(L/2)) / L)

    save_path = 'Solutions/'
    save_name = save_path + 'meanfreq_noOGE'
    save_variables_name = save_name + "_third_node.pkl"
    with open(save_variables_name, 'wb') as handle:
        pickle.dump({'MNF': {'EKF_biorbd': MNF_EKF_biorbd_sum},
                     'FFT': {'EKF_biorbd': FFT_EKF_biorbd_sum, 'freq': fft_freq}},
                    handle, protocol=3)
