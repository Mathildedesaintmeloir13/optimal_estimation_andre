...
from adjust_Kalman import adjust_Kalman, shift_by_2pi
...
if __name__ == "__main__":
    ...
    q_ref_biorbd = shift_by_2pi(biorbd_model, q_ref_biorbd[:, ::step_size])
    ...
    q_ref = q_ref_biorbd
    qdot_ref = qdot_ref_biorbd
    qddot_ref = qddot_ref_biorbd
    ...
    markers_reordered, markers_idx_ref = reorder_markers(biorbd_model, c3d, frames, step_size)
    ...
    with open(save_variables_name, 'wb') as handle:
        pickle.dump({'states': states, 'controls': controls, 'params': params, 'gravity': gravity, 'mocap': markers_reordered},
                    handle, protocol=3)
