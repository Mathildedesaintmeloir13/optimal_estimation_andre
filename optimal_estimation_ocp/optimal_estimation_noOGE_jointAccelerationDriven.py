...
sys.path.insert(1, '/home/mickael/Documents/Eve/Andre_bioptim_gravity_optimization/BiorbdOptim-gravity_optimization/examples/optimal_gravity_ocp/')
...
def root_explicit_dynamic(states, controls, parameters, nlp):
    ...
    q, qdot, qddot_joints = DynamicsFunctions.dispatch_q_qdot_tau_data(states, controls, nlp)
    ...
    Q_sym = cas.MX.sym('Q_sym', q.shape[0])
    Qdot_sym = cas.MX.sym('Qdot_sym', qdot.shape[0])
    Qddot_joints_sym = cas.MX.sym('Qddot_joints_sym', qdot.shape[0] - nb_root)

    mass_matrix_nl_effects = nlp['model'].InverseDynamics(Q_sym, Qdot_sym, cas.vertcat(cas.MX.zeros((nb_root, 1)), Qddot_joints_sym)).to_mx()[:6]
    mass_matrix_nl_effects_func = cas.Function("mass_matrix_nl_effects_func", [Q_sym, Qdot_sym, Qddot_joints_sym], [mass_matrix_nl_effects[:nb_root]])

    mass_matrix = nlp['model'].massMatrix(Q_sym).to_mx()
    M_66 = mass_matrix[:nb_root, :nb_root]
    M_66_func = cas.Function("M66_func", [Q_sym], [M_66])
...
def configure_qddot_joint(nlp, as_states: bool, as_controls: bool):
    ...
    Problem.configure_new_variable("qddot_joint", name_qddot_joint, nlp, as_states, as_controls)
...
def custom_configure_root_explicit(ocp, nlp):
    ...
    nb_root = nlp["model"].nbRoot()
    name_qddot_joint = [str(i + nb_root) for i in range(nlp["model"].nbQddot() - nb_root)]
    Problem.configure_new_variable("qddot_joint", name_qddot_joint, nlp, as_states=False, as_controls=True)

    Problem.configure_forward_dyn_func(ocp, nlp, root_explicit_dynamic)
...
def prepare_ocp(biorbd_model, final_time, number_shooting_points, markers_ref, q_init, qdot_init, qddot_init, xmin, xmax):
    # --- Options --- #
    ...
    U_bounds = BoundsList()
    U_bounds.add(Bounds(min_bound=[torque_min] * (n_tau-n_root), max_bound=[torque_max] * (n_tau-n_root)))

    U_init = InitialConditionsList()
    # qddot_init = np.zeros(qddot_init.shape)
    U_init.add(qddot_init, interpolation=InterpolationType.EACH_FRAME)

    tau_mapping = BidirectionalMapping(
                                       Mapping([i for i in range(biorbd_model.nbQ() - biorbd_model.nbRoot())]),
                                       Mapping([i for i in range(biorbd_model.nbRoot(), biorbd_model.nbQ())])
                                      )
...
if __name__ == "__main__":
    ...
    data_path = '/home/mickael/Documents/Eve/OptimalEstimation/Andre_data/'
    ...
    if GENERATE_GRAPHS:
        figure_save_path = "/home/mickael/Documents/Eve/OptimalEstimation/figures/Figures_vieuxCodesAndre/"
    ...
    if GENERATE_OPTIM:
        ocp = prepare_ocp(
            ...
            markers_ref=markers_rotated, q_init=q_ref, qdot_init=qdot_ref, qddot_init=qddot_ref[6:, :-1],
            xmin=xmin, xmax=xmax)
